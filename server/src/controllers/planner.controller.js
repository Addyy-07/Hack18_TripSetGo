...
exports.generatePlan = asyncHandler(async (req, res) => {
  ...
  if (!plan) {
    ...
    // Use the existing fallback engine (adapts input to its expected shape)
    plan = fallback.generatePlan({
      source:       'Your Location',
      destination:  input.destination,
      startDate:    new Date().toISOString(),
      endDate:      new Date(Date.now() + input.days * 86400000).toISOString(),
      budget:       input.budget,
      numTravelers: 1,
      groupType:    'solo',
